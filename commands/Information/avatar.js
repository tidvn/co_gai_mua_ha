  ...
  run: async (client, message, args, plusArgs, cmdUser, text, prefix) => {
    ...
    try {
      ...
      message.reply({embeds: [new Discord.MessageEmbed()
        .setAuthor(` ${user.tag}`, user.displayAvatarURL({ size: 4096, dynamic: true}), "https://discord.gg/JNGKH5wVHk")
        .setColor(ee.color)
        .addField("❱ PNG",`[\`LINK\`](${user.displayAvatarURL({size: 4096,format: "png"})})`, true)
        .addField("❱ JPEG",`[\`LINK\`](${user.displayAvatarURL({size: 4096,format: "jpg"})})`, true)
        .addField("❱ WEBP",`[\`LINK\`](${user.displayAvatarURL({size: 4096,format: "webp"})})`, true)
